...
import { startWorkersSyncWorker } from '@crm/sync-scheduler';
...
async function bootstrap() {
  ...
  const worker = startWorkersSyncWorker({ providerFactory });
...
bootstrap().catch((err) => {
  // eslint-disable-next-line no-console
  console.error('Failed to start Workers Sync Worker', err);
  process.exit(1);
});
