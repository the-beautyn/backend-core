...
import { startInitialSyncWorker } from '@crm/sync-scheduler';
...
async function bootstrap() {
  ...
  const worker = startInitialSyncWorker({ providerFactory });
  ...
  console.log('Initial Sync Worker started');
}

bootstrap().catch((err) => {
  // eslint-disable-next-line no-console
  console.error('Failed to start Initial Sync Worker', err);
  process.exit(1);
});
