...
import { startServicesSyncWorker } from '@crm/sync-scheduler';
...
async function bootstrap() {
  ...
  const worker = startServicesSyncWorker({ providerFactory });
  ...
  console.log('Services Sync Worker started');
}

bootstrap().catch((err) => {
  // eslint-disable-next-line no-console
  console.error('Failed to start Services Sync Worker', err);
  process.exit(1);
});
